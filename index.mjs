        ...
        for (const link of mediaUrl.element.nav.meialLink) {
            switch (link["@_type"]) {
                case "mp4":
                    if (link["@_file"]) {
                        batch.push(
                            `ffmpeg -n -i "${mediaUrl.element.info.mp4Url_2}_definst_/mp4:${mediaUrl.element.info.mp4Sub_2}${link["@_file"]}/playlist.m3u8" -c copy "${link["@_file"]}"`
                        );
                ...
                case "mp3":
                    const mp3 = await fetch(
                        `http://www.safetyedu.net/econtents/Content/NEW_CONTENTS/${lecture.path}/common/mp3/${link["@_file"]}`
                    );
